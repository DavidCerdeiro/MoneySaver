...
export const createSpendingSchema = (t: any) =>
  ...
  // Validaciones condicionales para gastos periódicos
  .refine((data) => {
  ...
  // Validación de que la fecha sea del mes actual
  .refine((data) => {
    if (!data.date) return true; // ya validaste que no esté vacío
    const spendingDate = new Date(data.date);
    const today = new Date();
    return (
      spendingDate.getFullYear() === today.getFullYear() &&
      spendingDate.getMonth() === today.getMonth()
    );
  }, {
    path: ["date"],
    message: t("domains.spending.errors.date.currentMonth"),
  });
